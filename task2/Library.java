...
//    private startp final int Max_Price-for biiol= 1000;
    ...
    public static void main(String[] args) {
        Book[] books = getBooks();
        displayBooks(books);
        int bookId = 0; // bood id to be modified
        books[bookId] = modifyBook(books[bookId], 200);
        displayBooks(books);
    }

    /**
     * This method allows modification of chosen book price
     *
     * @param book the book
     * @param price new book price
     * @return
     */
    private static Book modifyBook(Book book, double price) {
        if (book != null) {
            book.setPrice(price);
        } else {
            System.out.println("Error: The book doesn't exist! Nothing can be modified");
        }
        return book;
    }
